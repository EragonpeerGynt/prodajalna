...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      stmt.run(polja.FirstName, polja.LastName, polja.Company, polja.Address, polja.City, polja.State, polja.Country, polja.PostalCode, polja.Phone, polja.Fax, polja.Email, 3); //stmt.run("", "", "", "", "", "", "", "", "", "", "", 3); 
      stmt.finalize();//stmt.finalize();
    } catch (err) {
      napaka2 = true;
    }
    if(napaka1 || napaka2) {
        textReturner = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
        odgovor.redirect("/prijava"); 
      }
      else {
        textReturner = "Stranka je bila uspešno registrirana.";
        odgovor.redirect("/prijava"); 
      }
    odgovor.end();
  });
...
streznik.get('/prijava', function(zahteva, odgovor) {
  vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        ...
        odgovor.render('prijava', {sporocilo: textReturner, seznamStrank: stranke, seznamRacunov: racuni});  
      }) 
    });
})
